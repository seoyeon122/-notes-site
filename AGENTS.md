...
- **内容授权**：朋友周报原文未经用户明确授权，**不得**写入或恢复仓库内容。当前 `issue-2.md` 正文是占位（"内容待授权后补全"）；完整原文只在本地 `参考文献/zhuanlan.md`。用户拿到授权并确认后，才恢复完整内容。
- **仓库保持 Private**；未授权前不要部署公网或改 Public。
...
2. 等用户拿到朋友授权 → 恢复 issue-2 完整内容 → 之后可公开仓库
