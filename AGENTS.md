...
> 🔒 **内部维护文档**：本文件是给项目维护者与本地 AI 协作者看的交接说明，**不是面向网站读者的内容**。访客请阅读 [README.md](README.md)。
...
- **内容授权**：朋友周报**已获授权收录**（第 1~6 期已全部收录上线）。但未经用户明确授权，**不得**收录未来其他期的原文；`参考文献/` 仍仅本地（.gitignore），内含 `zhuanlan-1.md` 至 `zhuanlan-6.md` 供重排版参考。
- **仓库已公开**（2026 用户决定），**勿改回 Private**；内容收录均已获授权（详见 README 版权声明）。
- Astro v7 API 变化多：改到不熟的 API 先查官方文档。
- 站名/文案只在 `src/consts.ts` 改。

## 8. 当前状态与 Roadmap（优先级从高到低）

**已完成**：内容模型（期数归档 1~6 期）、红黑双主题（暗色全站 + 浅色阅读页）、动效地基、首页视频影院、响应式 WebP 大背景、git 公开仓库、文档（README/LEARNING/DEPLOY/weekly-template/本文件）。

**待办**：
1. ~~骨架升级~~（✅ **已全部完成**：速览卡/时间线目录/上一篇下一篇/本周数据块/切片卡片，模板已升级）
2. ~~等授权 → 恢复 issue-2~~（已完成：已获授权，第 2 期完整内容已恢复）；~~等用户决定 → 公开仓库~~ ✅ **已公开**（2026 用户决定）
