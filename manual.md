...
> 🔒 **内部维护文档**：给项目维护者的手操指南，**不是面向网站读者的内容**；访客请看 [README.md](README.md)。
...
- 首页第一版左栏（"阅读最新一期"按钮下方）有"近期更新"栏，是**数据驱动**的：在 `src/pages/index.astro` 顶部的 `recentUpdates` 数组里**加一条**即可。**只显示最新 4 条**（`slice(0, 4)`），超出的旧条目自动不显示，无需手动删。
- 加条目的格式：
  ```js
  const recentUpdates = [
...
- **首页电台版**：`src/pages/index.astro` 的 `<section class="radio">`，由 `src/components/RadioAlbums.astro` 先过滤 `hidden`，再取前 4 期显示封面（`filter(...).slice(0, 4)`）；新增一期时最旧的一张自动退出首页，全部期数仍保留在 `/radio` 归档页。首页；封面说明直接读取 `album`，默认隐藏，鼠标悬停或键盘聚焦时从底部显示。电台归档页的常驻文字不受这组首页样式影响。
