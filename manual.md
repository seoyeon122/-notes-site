...
- **左区**：`radio-kicker`（眉标）、`radio-title`（标题）、`radio-desc`（介绍，当前占位文案"每周一清晨…"）、`radio-cta`（按钮）；正式文案待填充
- **右区**：`.radio-grid` 里 4 个 `.radio-slot` 正方形占位（无圆角、虚线框、标「施工中」）；后续把 4 张电台图片填进这 4 个格子里
...
- 左侧栏在 `src/components/Rail.astro`（宽 80px，固定贴左，全站共用）：
  ...
  - 竖排站名、竖排信号灯装饰也在里面
- **移动端自动变顶栏**（≤820px 宽）：左侧栏自动变成顶部横栏（站名横排、导航横排、激活红下划线），信号灯隐藏；正文让位从"左边距"自动改成"上边距"。不用手动维护，改样式时注意别破坏 `@media (max-width: 820px)` 里的顶栏规则（在 `Rail.astro` 和 `global.css` 底部）
