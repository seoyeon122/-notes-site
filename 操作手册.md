...
- **移动端自动变顶栏**（≤820px 宽）：左侧栏自动变成顶部横栏（站名横排、导航横排、激活红下划线、切换按钮靠右），信号灯隐藏；正文让位从"左边距"自动改成"上边距"。不用手动维护，改样式时注意别破坏 `@media (max-width: 820px)` 里的顶栏规则（在 `Rail.astro` 和 `global.css` 底部）
